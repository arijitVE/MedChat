...
type UserActionVariables = { userId: string; nextActive: boolean };
type UserActionContext = { previousUsers?: PaginatedResponse<UserListItem> };
  ...
  return (
    ...
    >
      {user ? (
        ...
        >
          <p className="text-sm text-clinical-text-secondary">Set a new password for {user.full_name}.</p>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" loading={isSubmitting} disabled={password.length < 8}>Reset</Button>
          </div>
        </form>
      ) : null}
    </Modal>
...
export default function UsersPage() {
  ...
  const users = useQuery({
    queryKey,
    queryFn: async () => (await adminApi.getUsers(filters)).data,
  ...
  const toggleActive = useMutation<UserListItem, ApiError, UserActionVariables, UserActionContext>({
    mutationFn: async ({ userId, nextActive }) => {
      const response = nextActive ? await adminApi.activateUser(userId) : await adminApi.deactivateUser(userId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Users" description="Review users and manage account access." />
          ...
          <TableBody>
            {(users.data?.items ?? []).map((user) => (
              <TableRow key={user.user_id}>
                ...
                <TableCell>{user.patient_uid ?? user.license_number ?? '-'}</TableCell>

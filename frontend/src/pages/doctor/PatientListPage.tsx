...
export default function PatientListPage() {
  ...
  const visiblePatients = shouldSearch ? searchResults.data ?? [] : [];
  const isLoading = shouldSearch ? searchResults.isLoading : false;
  const isError = shouldSearch ? searchResults.isError : false;
  ...
  if (isError) {
    return <RetryPanel onRetry={() => void searchResults.refetch()} message={normalizeApiError(searchResults.error).message} />;
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="grid gap-3 md:grid-cols-[1fr_auto_auto]">
          ...
          <input
            ...
            className="rounded-md border border-clinical-border px-3 py-2 text-sm outline-none focus:border-clinical-primary focus:ring-2 focus:ring-clinical-primary-light"
            aria-label="Patient UID for assignment"
          />
          <Button
            loading={createAssignment.isPending}
            disabled={!patientUid.trim()}
            onClick={() => {
              createAssignment.mutate({ patient_uid: patientUid.trim() });
            }}
          >
            Add Patient
          </Button>
      ...
      <Card>
        {isLoading ? (
          ...
          <EmptyState title={shouldSearch ? 'No patients found' : 'Search patients by name or Patient UID'} />
        ) : (
